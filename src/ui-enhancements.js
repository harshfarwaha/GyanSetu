(() => {
  ...
  async function openWindow(title,setup){
    ...
    const head=document.createElement('header');head.className='uiWindowHead';const titleWrap=document.createElement('div');titleWrap.innerHTML=`<h2>${escapeHtml(title)}</h2><p>Browse GyanSetu's available open books</p>`;const close=makeCloseButton();head.append(titleWrap,close);
    ...
    const state=createTemporaryContent(body);if(!state){overlay.remove();document.body.classList.remove('gyansetu-ui-locked');return null;}
    overlay.__gyansetuState=state;close.onclick=()=>closeWindow(overlay,state);overlay.onclick=(event)=>{if(event.target===overlay)closeWindow(overlay,state);};
  ...
  function openSearchWindow(form,originalSubmit,event){
    event.preventDefault();if(document.querySelector('.uiOverlay'))return;const query=form.querySelector('#q')?.value?.trim();if(!query){form.querySelector('#q')?.focus();return;}
    openWindow(`Search: ${query}`,async(body)=>{
      const tools=document.createElement('div');tools.className='uxSearchTools';tools.innerHTML=`<input id="uxSearchInput" value="${escapeHtml(query)}" aria-label="Search books" placeholder="Search titles, authors, or genres"><select id="uxGenre"><option value="">All genres</option><option>Romance</option><option>Mystery & Detective</option><option>Fiction, Classics & Literature</option><option>Adventure</option><option>Science Fiction & Fantasy</option><option>Storybooks & Children</option><option>Science, Math & Technology</option><option>History, Biography & Travel</option><option>Philosophy, Religion & Ideas</option><option>Indian</option><option>Poetry</option><option>Shayari & Punjabi Poetry</option></select><select id="uxLanguage"><option value="">All languages</option><option value="English">English</option><option value="Hindi">Hindi</option><option value="Punjabi">Punjabi</option><option value="Urdu">Urdu</option></select><button id="uxSearchButton" type="button">Search</button>`;body.prepend(tools);
      const hint=document.createElement('p');hint.className='uxSearchHint';hint.textContent='Search results stay directly below the search controls.';body.insertBefore(hint,document.getElementById('content'));
      const execute=()=>{const q=document.getElementById('uxSearchInput').value.trim();if(!q)return;const genre=document.getElementById('uxGenre').value;const language=document.getElementById('uxLanguage').value;runOriginalSearch(originalSubmit,[q,genre,language].filter(Boolean).join(' '));};
      document.getElementById('uxSearchButton').onclick=execute;document.getElementById('uxSearchInput').addEventListener('keydown',(e)=>{if(e.key==='Enter')execute();});execute();
    });
  }

  function openGenreWindow(button,originalClick,event){
    event.preventDefault();const title=button.closest('.shelf')?.querySelector('h2')?.textContent?.trim()||'Genre';
    openWindow(title,async(body)=>{const tools=document.createElement('div');tools.className='uxCollectionTools';const input=document.createElement('input');input.placeholder=`Search within ${title}…`;input.setAttribute('aria-label',`Search within ${title}`);const label=document.createElement('span');label.className='muted';tools.append(input,label);body.prepend(tools);await Promise.resolve(originalClick.call(button,{preventDefault(){}}));const temp=document.getElementById('content');await waitForResults(temp);const results=temp.querySelector('.results');if(!results)return;const count=()=>{const visible=[...results.querySelectorAll('.bookCard')].filter(c=>c.style.display!=='none').length;label.textContent=`${visible} shown`;};input.oninput=()=>{const term=input.value.trim().toLowerCase();results.querySelectorAll('.bookCard').forEach(card=>{card.style.display=!term||card.textContent.toLowerCase().includes(term)?'':'none';});count();};count();});
  }

  function enhanceDetails(){
    const details=document.querySelector('.modal .details');if(!details||details.querySelector('.uxFavorite'))return;const title=details.querySelector('h2')?.textContent?.trim()||pendingBook?.title||'';const author=details.querySelector('.byline')?.textContent?.trim()||pendingBook?.author||'';const cover=details.querySelector('.detailCover img')?.getAttribute('src')||pendingBook?.cover||'';const sourceLink=details.querySelector('.sourceLink')?.getAttribute('href')||'';const id=pendingBook?.id||`detail-${title.toLowerCase().replace(/[^a-z0-9]+/g,'-')}`;const record={id,title,author,cover,source:pendingBook?.source||'',readUrl:sourceLink};
    const button=document.createElement('button');button.type='button';button.className='uxFavorite';button.dataset.favoriteId=id;button.textContent=isFavorite(id)?'♥ Saved to My Library':'♡ Save to My Library';if(isFavorite(id))button.classList.add('isSaved');button.onclick=()=>toggleFavorite(record);const primary=details.querySelector('.primary');if(primary)primary.insertAdjacentElement('afterend',button);else details.appendChild(button);pendingBook={...record};
  }
  ...
  function enhanceInteractions(){
    document.addEventListener('click',(event)=>{const card=event.target.closest('[data-id]');if(card)pendingBook=cardRecord(card);if(event.target.closest('.primary')&&pendingBook)setTimeout(()=>saveRecord(RECENT_KEY,{...pendingBook,openedAt:Date.now()}),120);},true);
    const observer=new MutationObserver(()=>{enhanceDetails();addBottomNav();enhanceTopActions();});observer.observe(document.body,{childList:true,subtree:true});
  }

  function enhance(){
    injectStyles();addBottomNav();enhanceTopActions();enhanceDetails();document.querySelectorAll('.shelf').forEach((shelf)=>shelf.classList.add('uxShelfReady'));
    const form=document.querySelector('.search');if(form&&!form.dataset.uiEnhanced){const originalSubmit=form.onsubmit;if(typeof originalSubmit==='function'){form.onsubmit=(event)=>openSearchWindow(form,originalSubmit,event);form.dataset.uiEnhanced='true';}}
