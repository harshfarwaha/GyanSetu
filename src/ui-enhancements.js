(() => {
  ...
  async function openWindow(title,setup){
    ...
    const state=createTemporaryContent(body);if(!state){overlay.remove();document.body.classList.remove('gyansetu-ui-locked');return null;}overlay.__gyansetuState=state;
    close.onclick=()=>closeWindow(overlay,state);overlay.onclick=(event)=>{if(event.target===overlay)closeWindow(overlay,state);};
    body.addEventListener('click',(event)=>{const card=event.target.closest('[data-id]');if(!card)return;const original=state.original.querySelector(`[data-id="${CSS.escape(card.dataset.id)}"]`);if(original){closeWindow(overlay,state);original.click();}});
