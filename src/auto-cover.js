(() => {
  ...
  async function resolve({ card, title, author, key: cacheKey }) {
    ...
    if (saved && Date.now() - saved.savedAt < TTL) {
      apply(card, saved.coverUrl);
      return;
    }
    try {
      const response = await fetch('/api/resolve-cover', {
  ...
  function apply(card, coverUrl) {
    ...
    const target = card.querySelector('.coverBtn .coverFallback');
    if (!target) return;
    const image = document.createElement('img');
    image.loading = 'lazy';
    image.decoding = 'async';
    image.src = coverUrl;
    image.alt = card.querySelector('h3')?.textContent?.trim() || '';
  ...
  function enqueue(card) {
    ...
    if (!title) return;
    const cacheKey = key(title, author);
    if (queued.has(cacheKey)) return;
    queued.add(cacheKey);
    queue.push({ card, title, author, key: cacheKey });
    runNext();
  }

  function scan(root = document) {
    root.querySelectorAll?.('.bookCard').forEach((card) => observer.observe(card));
  }
  ...
  const mutationObserver = new MutationObserver((mutations) => mutations.forEach((mutation) => mutation.addedNodes.forEach((node) => {
    if (node.nodeType === 1) {
      if (node.matches?.('.bookCard')) observer.observe(node);
      scan(node);
    }
  })));
