...
function render() {
  ...
  app.innerHTML = `<div class="app"><header class="topbar"><a class="brand" href="#" aria-label="GyanSetu home"><img class="brandLogo" src="src/assets/gyansetu-logo.svg" alt="GyanSetu logo"><div><b>GyanSetu</b><span>Digital Library</span></div></a><form class="search"><span>⌕</span><input id="q" placeholder="Search comics, manga, storybooks, encyclopedias..."><button>Search</button></form><nav class="topActions"><button class="navBtn" id="historyBtn">History</button><button class="accountBtn" id="loginBtn">${currentUser ? `<span>${userInitials()}</span>${esc(currentUser.name)}` : 'Continue with Google'}</button><button class="iconBtn" id="theme" aria-label="Toggle theme">${dark ? '☀' : '☾'}</button></nav></header><main><section class="hero"><div class="heroText"><p class="eyebrow">✦ open web reading room</p><h1>Read free books without broken links.</h1><p>GyanSetu now includes every major reading lane: comics, manga, webcomics, storybooks, encyclopedias, reference works, textbooks, classics, and research books from legal open-web or official free sources.</p><div class="heroActions"><button data-search="Indian">Open Indian books</button><button class="ghost" data-search="comics manga storybooks encyclopedia">Explore all types</button></div></div><aside class="device"><div class="deviceTop">Fast reading desk <span>Reliable links</span></div><div class="gridMini" id="featured"></div></aside></section><section class="historyPanel" id="history"><div><p class="eyebrow">Reading history</p><h2>Pick up where you left off</h2></div><div id="historyList"></div></section><section class="resources" id="resources"></section><div id="content"></div></main></div>`;
