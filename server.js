...
async function fetchJson(url) {
  const response = await fetch(url, { headers: { 'User-Agent': 'GyanSetu/1.0 (open digital library)' } });
...
async function getCandidates(title, author) {
  ...
  if (openLibrary.status === 'fulfilled') {
    for (const item of openLibrary.value.docs || []) {
      if (!item.cover_i) continue;
      const itemTitle = item.title || '';
      const itemAuthor = (item.author_name || []).join(', ');
      const titleScore = similarity(title, itemTitle);
      const authorScore = author === 'Unknown author' ? 1 : similarity(author, itemAuthor);
      if (titleScore >= 0.65 && (authorScore >= 0.45 || author === 'Unknown author')) {
        candidates.push({ source: 'Open Library', title: itemTitle, author: itemAuthor, year: item.first_publish_year || null, coverUrl: `https://covers.openlibrary.org/b/id/${item.cover_i}-L.jpg`, titleScore, authorScore });
      }
    }
  }
  if (googleBooks.status === 'fulfilled') {
    for (const item of googleBooks.value.items || []) {
      const info = item.volumeInfo || {};
      const image = info.imageLinks?.thumbnail || info.imageLinks?.smallThumbnail;
      if (!image) continue;
      const itemTitle = info.title || '';
      const itemAuthor = (info.authors || []).join(', ');
      const titleScore = similarity(title, itemTitle);
      const authorScore = author === 'Unknown author' ? 1 : similarity(author, itemAuthor);
      if (titleScore >= 0.65 && (authorScore >= 0.45 || author === 'Unknown author')) {
        candidates.push({ source: 'Google Books', title: itemTitle, author: itemAuthor, year: info.publishedDate || null, coverUrl: image.replace(/^http:/, 'https:'), titleScore, authorScore });
      }
    }
  }
  const unique = new Map();
  for (const candidate of candidates) unique.set(candidate.coverUrl, candidate);
...
async function chooseWithGemini(title, author, candidates) {
  ...
  const response = await fetch('https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash-lite:generateContent', {
    ...
    body: JSON.stringify({
      ...
      generationConfig: {
        ...
        responseSchema: { type: 'OBJECT', properties: { candidateIndex: { type: 'INTEGER' }, confidence: { type: 'NUMBER' }, reason: { type: 'STRING' } }, required: ['candidateIndex', 'confidence', 'reason'] }
      }
    })
  });
  ...
  if (!Number.isInteger(index) || index < 0 || index >= candidates.length || confidence < 0.82) return null;
...
async function resolveCover(title, author) {
  ...
  try {
    ...
    if (candidates.length === 1 && candidates[0].titleScore >= 0.9 && candidates[0].authorScore >= 0.7) value = candidates[0];
    else value = await chooseWithGemini(title, author, candidates);
  } catch (error) {
    console.error('Cover resolver:', error.message);
  }
  const result = value ? { coverUrl: value.coverUrl, source: value.source, matchedTitle: value.title, matchedAuthor: value.author } : null;
...
const server = http.createServer(async (req, res) => {
  try {
    if (req.method === 'POST' && req.url === '/api/resolve-cover') {
      const body = await readBody(req);
      const title = String(body.title || '').trim();
      const author = String(body.author || 'Unknown author').trim();
      if (!title) return send(res, 400, { error: 'Book title is required.' });
      return send(res, 200, await resolveCover(title, author));
    }
